...
<div class="box">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
        <h2>Manajemen Petugas Perpustakaan</h2>
    ...
    <?php if($pesan=='berhasil'){ echo "<div class='alert alert-success'>✅ Operasi berhasil.</div>"; }
          if($pesan=='hapus'){ echo "<div class='alert alert-danger'>🗑️ Petugas dihapus.</div>"; }
    ?>
        ...
        <form method="post" action="proses_petugas.php">
            ...
            <div class="form-grid">
                ...
                <div class="full" style="display:flex;justify-content:flex-end;gap:8px;margin-top:6px">
                    <button type="submit" class="btn btn-tambah">Simpan Petugas</button>
                    <a href="petugas.php" class="btn btn-back">Batal</a>
                </div>
            </div>
        </form>
        <?php exit(); }
    ...
    if(isset($_GET['action']) && $_GET['action']=='edit' && isset($_GET['id'])){
        $id = intval($_GET['id']);
        $q = mysqli_query($koneksi, "SELECT * FROM user WHERE UserID='$id' AND Level IN ('petugas','admin')");
        if(mysqli_num_rows($q)==0){ echo "<div class='alert alert-danger'>Petugas tidak ditemukan.</div>"; exit(); }
        ...
        <form method="post" action="proses_petugas.php">
            ...
            <div class="form-grid">
                ...
                <div class="field">
                    <label>Level/Hak Akses</label>
                    <select name="Level" required>
                        <option value="petugas" <?php echo ($row['Level']=='petugas')?'selected':''; ?>>Petugas Perpustakaan</option>
                        <option value="admin" <?php echo ($row['Level']=='admin')?'selected':''; ?>>Administrator</option>
                    </select>
                </div>
                <div class="full" style="display:flex;justify-content:flex-end;gap:8px;margin-top:6px">
                    <button type="submit" class="btn btn-edit">Perbarui Petugas</button>
                    <a href="petugas.php" class="btn btn-back">Batal</a>
                </div>
            </div>
        </form>
    ...
    $sql = "SELECT * FROM user WHERE Level IN ('petugas','admin') ORDER BY NamaLengkap ASC";
    $res = mysqli_query($koneksi, $sql);
    ?>

    <div style="display:flex;gap:8px;margin-bottom:12px;">
        <a href="petugas.php?action=add" class="btn btn-tambah">➕ Tambah Petugas Baru</a>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><span style="background:#eef2f7;padding:4px 8px;border-radius:4px;font-size:12px;font-weight:600"><?php echo ucfirst($r['Level']); ?></span></td>
                <td><?php echo htmlspecialchars(substr($r['Alamat'], 0, 50), ENT_QUOTES) . (strlen($r['Alamat'])>50?'...':''); ?></td>
                <td>
                    <a href="petugas.php?action=edit&id=<?php echo $r['UserID']; ?>" class="btn btn-edit">Edit</a>
                    <a href="hapus_petugas.php?id=<?php echo $r['UserID']; ?>" class="btn btn-hapus" onclick="return confirm('Hapus petugas ini?')">Hapus</a>
                </td>
            </tr>
            <?php } } else { echo "<tr><td colspan='7' style='text-align:center;color:#666'>Belum ada petugas terdaftar.</td></tr>"; } ?>
        </tbody>
    </table>
    </div>
</div>
